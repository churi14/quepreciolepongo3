...
export default function Home() {
  ...
  const renderInternalContent = () => {
    ...
    // 1. BIENVENIDA (ESTILO ORIGINAL GRANDE)
    if (viewState === "welcome") {
        return (
            <div className="flex flex-col h-full items-center justify-center p-8 text-center animate-in fade-in zoom-in-95 duration-500">
                <div className="mb-10 transform hover:scale-105 transition-transform duration-500">
                    <img src="/logo.png" alt="Que Precio Le Pongo" className="h-24 md:h-28 w-auto object-contain mx-auto drop-shadow-sm" />
                </div>
                <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-6 tracking-tight max-w-2xl leading-tight">
                    Dejá de adivinar precios.<br/>Empezá a ganar plata.
                </h1>
                ...
                <Button 
                    size="lg" 
                    className="rounded-full px-12 py-7 text-lg bg-blue-600 hover:bg-blue-700 shadow-xl hover:shadow-2xl transition-all hover:-translate-y-1 font-bold"
    ...
    // 2. PERFIL: TIPO (AHORA CON HEADER Y FOOTER)
    if (viewState === "profile-type") {
        return (
            <div className="flex flex-col h-full animate-in slide-in-from-right-8 duration-500">
                {/* Header Interno */}
                <div className="p-6 flex justify-between items-center border-b border-slate-50">
                    <img src="/logo.png" alt="Logo" className="h-6 w-auto opacity-80" />
                    <Button variant="ghost" className="text-xs text-slate-400" onClick={() => setViewState("welcome")}>Volver</Button>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
                        <button onClick={() => { setBusinessType("solo"); setViewState("profile-industry"); }} className="group p-6 border-2 border-slate-100 rounded-2xl hover:border-blue-500 hover:bg-blue-50/50 transition-all text-left bg-white">
                        ...
                        <button onClick={() => { setBusinessType("team"); setViewState("profile-industry"); }} className="group p-6 border-2 border-slate-100 rounded-2xl hover:border-purple-500 hover:bg-purple-50/50 transition-all text-left bg-white">
                ...
                <div className="p-4 text-center border-t border-slate-50">
                    <span className="text-[10px] text-slate-300 font-medium">EN RED CONSULTORA</span>
                </div>
            </div>
        );
    }

    // 3. PERFIL: INDUSTRIA (CON HEADER Y FOOTER)
    if (viewState === "profile-industry") {
        ...
        return (
            <div className="flex flex-col h-full animate-in slide-in-from-right-8 duration-500">
                ...
                <div className="p-6 flex justify-between items-center border-b border-slate-50">
                    <img src="/logo.png" alt="Logo" className="h-6 w-auto opacity-80" />
                    <Button variant="ghost" className="text-xs text-slate-400" onClick={() => setViewState("profile-type")}>Atrás</Button>
                </div>
    ...
    // 4. CALCULADORA (WIZARD COMPLETO)
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-500">
            {/* Header del Wizard */}
            <div className="flex justify-between items-center p-6 border-b border-slate-100">
                <div className="flex flex-col items-start">
                    <img src="/logo.png" alt="Logo" className="h-5 w-auto object-contain mb-1 opacity-80" />
            ...
            <div className="hidden md:flex w-80 bg-white rounded-[2rem] shadow-xl flex-col h-full animate-in slide-in-from-right duration-700 ring-1 ring-slate-900/5 overflow-hidden">
                ...
                <div className="p-6 flex-1 overflow-y-auto bg-white">
                    <div className="space-y-4 text-sm">
                        <div className="flex justify-between items-center group">
                            <span className="text-slate-500">Materiales y Envases</span>
                            <span className="font-medium text-slate-900 bg-slate-50 px-2 py-1 rounded border border-slate-100" suppressHydrationWarning>{formatMoney(totalMaterials)}</span>
                        </div>
                        <div className="flex justify-between items-center group">
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100">
                    <div className="text-center mb-5">
                        ...
                        <div className="text-4xl font-black text-slate-900 my-2 tracking-tighter" suppressHydrationWarning>{formatMoney(finalPrice)}</div>
                        {profitAmount > 0 && <div className="inline-flex items-center gap-1 text-xs font-bold text-green-700 bg-green-100 px-3 py-1 rounded-full" suppressHydrationWarning>+{formatMoney(profitAmount)} ganancia</div>}
                    </div>
                    <LeadCaptureModal onSuccess={(userData) => generatePDF({ ingredients, laborCost, fixedCost: fixedCostPerUnit, totalCost, profit: profitAmount, finalPrice, productName: userData.name })} triggerButton={<Button className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold h-12 rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-95">Descargar Reporte</Button>} />
                </div>
            </div>
        )}
      </div>
