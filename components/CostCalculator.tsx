...
export default function CostCalculator({ businessType, industry, onBack }: CostCalculatorProps) {
  ...
  const [sellingRate, setSellingRate] = useState(0); // % del precio (impuestos + plataforma + medio de cobro)
  ...
  // El paso "Canal de Venta" ocurre antes de que definas el precio final, así que
  // usamos el precio final una vez que existe (finalPrice) y mientras tanto una
  // estimación (2x costo de producción). Como sellingExpenses se recalcula acá
  // en cada render a partir de finalPrice, en cuanto el usuario ingresa un
  // precio de mercado real en el último paso, las comisiones/impuestos se
  // recalculan solos con ese precio (antes quedaban congeladas con la estimación).
  const priceForCommissions = finalPrice || productionCost * 2;
  const sellingExpenses = priceForCommissions * sellingRate;
  ...
  const calculatorSteps = [
    ...
    { title: "Canal de Venta", subtitle: "Impuestos y comisiones.", component: <Commercialization productPrice={priceForCommissions} onRateChange={setSellingRate} /> },
