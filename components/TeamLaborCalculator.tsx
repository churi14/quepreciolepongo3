...
type Employee = {
  ...
  hoursPerMonth: number; // Horas mensuales que aporta esta persona (para prorratear el costo por minuto del equipo)
};
...
export default function TeamLaborCalculator({ onCostChange }: Props) {
  ...
  const [monthlyHours, setMonthlyHours] = useState("160"); // Horas al mes dedicadas a producción (aplica a ambas modalidades de pago)

  // CÁLCULO DE COSTO TOTAL
  const totalMonthlyCost = employees.reduce((sum, emp) => sum + emp.totalCost, 0);
  // Antes se dividía siempre por 160hs fijas sin importar cuánta gente había
  // cargada, así que al sumar más personal el costo por unidad se disparaba en
  // vez de reflejar la capacidad real del equipo. Ahora sumamos las horas
  // mensuales reales de cada persona cargada.
  const totalMonthlyHours = employees.reduce((sum, emp) => sum + emp.hoursPerMonth, 0);

  // TIEMPO DE PRODUCCIÓN (Para prorratear)
  const [productionTime, setProductionTime] = useState(30); // Minutos por unidad
  
  // Costo por Minuto del Equipo = (Costo Mes Equipo / Horas Mes Totales del Equipo) / 60
  const costPerMinute = totalMonthlyHours > 0 ? (totalMonthlyCost / totalMonthlyHours) / 60 : 0;
  ...
  // Horas mensuales estándar para una jornada completa (48hs/semana) y media jornada (24hs/semana)
  const HOURS_FULL_TIME = 48 * 4.2;
  const HOURS_PART_TIME = 24 * 4.2;
  ...
  const addEmployee = () => {
    ...
    if (empType === "blanco") {
        ...
        newEmp = {
            ...
            hoursPerMonth: modality === "full" ? HOURS_FULL_TIME : HOURS_PART_TIME,
        };
    } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-in fade-in">
                ...
                <div className="space-y-1">
                    <label className="text-xs text-slate-500">
                        ...
                        {monoFreq === "fijo" && <span className="text-slate-400"> (para prorratear el costo)</span>}
                    </label>
             ...
             <div key={emp.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg shadow-sm">
                 <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-1">
                        ...
                        {" · "}{Math.round(emp.hoursPerMonth)}hs/mes
                    </div>
                 </div>
          ...
          <div className="bg-slate-100 p-4 rounded-xl space-y-4">
              ...
              <div className="border-t border-slate-200 pt-4">
                  ...
                  <div className="flex items-center gap-2 bg-blue-100 p-2 rounded text-blue-800 text-xs">
                     ...
                     <span>El costo se calcula dividiendo la nómina mensual por el total de horas mensuales cargadas de tu equipo ({Math.round(totalMonthlyHours)}hs) y multiplicando por el tiempo de producción.</span>
                  </div>
              </div>
